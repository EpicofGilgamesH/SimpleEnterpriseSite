...
function findUnSaved() {
    ...
    $.ajax({
        ...
        data: { 'query': query },
        dataType: 'json',
        url: "/UserManage/GetUserByNameOrId",
...
function xiala(data) {
    ...
    for (var i = 0; i < data.length; i++) {
        var o = new Option(data[i].UserId, data[i].RealName + "(" + data[i].UserId + ")");
        $xialaSELECT.append(o);
    }
    if (firstTimeYes == 1) {
...
function initXialaSelect() {
    $xialaSELECT = $('<div></div>').css('position', 'absolute').css('overflow-y', 'scroll').css('overflow-x', 'hidden').css('border', '1px solid #809DB9').css('border-top', 'none').css
        ('top', $('#province').position().top + $('#province').height() + 10 + 'px').css('width', '162px').css('z-index', '101').css('width', '152px').css('background', '#fff').css('height', '200px').css('max-height', '600px').attr("id", "optionid");
    $('#province').after($xialaSELECT);
    //选项层的鼠标移入移出样式 
    $xialaSELECT.mouseover(function (event) {
        if ($(event.target).get(0).tagName == 'OPTION') {
            //if ($(event.target).attr('id') == 'optionid') {
            //移入时背景色变深，字色变白 
